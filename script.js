...
/* ===== اختيار صورة البروفايل عند التسجيل ===== */
...
async function loadProfile(){
  ...
  document.getElementById('profileUsername').textContent = '@' + (data.username || user.email.split('@')[0]);
  document.getElementById('bioInput').value = data.bio || '';
  selectedAvatar = data.avatar || '🌙';

  const photoEl = document.getElementById('homeAvatarPhoto');
  const pickerEl = document.getElementById('avatarPicker');
  if(selectedAvatar && selectedAvatar.startsWith('http')){
    photoEl.src = selectedAvatar;
    ...
    document.querySelectorAll('.avatar-opt').forEach(b => {
      b.classList.toggle('selected', b.getAttribute('data-emoji') === selectedAvatar);
    });
  }
}
...
async function saveProfile(){
  ...
  const { error } = await supabase
    .from('profiles')
    .update({ bio: bio, avatar: selectedAvatar })
    .eq('id', currentUserId);
...
async function completeProfile(){
  ...
  try{
    ...
    if(selectedAvatarFile && user){
      ...
      const { error: upErr } = await supabase.storage.from('avatars').upload(path, selectedAvatarFile, { upsert: true });
      if(!upErr){
